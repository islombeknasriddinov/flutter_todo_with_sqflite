...
class MyAppBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  _AppBarState _appBarState;

  @override
  _AppBarState createState() {
    this._appBarState = _AppBarState();
    return this._appBarState;
  }

  void setAppBarState(_AppBarState appBarState) {
    this._appBarState = appBarState;
  }

  void setLeading(Widget widget, {bool notify}) {
    this.leading = widget;
    if (notify != null && notify) {
      this._appBarState.notifyDataChange();
    }
  }

  void setTitle(Widget widget, {bool notify}) {
    this.title = widget;
    if (notify != null && notify) {
      this._appBarState.notifyDataChange();
    }
  ...
  void setActions(List<Widget> actions, {bool notify}) {
  ...
  void setBackgroundColor(Color color, {bool notify}) {
  ...
  void setIconTheme(Color color, {bool notify}) {
    this.iconTheme = color == null ? color : IconThemeData(color: color);
    if (notify != null && notify) {
      this._appBarState.notifyDataChange();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      ...
      child: AnnotatedRegion<SystemUiOverlayStyle>(
        value: overlayStyle,
        child: Material(
          color: widget.backgroundColor ?? appBarTheme.color ?? theme.primaryColor,
