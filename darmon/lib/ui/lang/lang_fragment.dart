...
class LangContentFragment extends ViewModelFragment<LangViewModel> {
  ...
  Widget onCreateWidget(BuildContext context) {
    ...
    final row = <Widget>[
      Padding(
          child: MyText(R.strings.lang.select_language, style: TS_LANG_TITLE()),
  ...
  Widget _buildItem(String lang, String title, bool hasDivider, void callback(String lang)) {
    ...
    if (hasDivider) {
      listColumn.add(Divider(color: Colors.black26, height: 1.5));
    }
    listColumn.add(ListTile(
      title: Center(
        child: MyText(title, style: TS_LANG()),
      ),
      onTap: () {
        callback.call(lang);
      },
  ...
  void setSelectLocale(String lang) {
    AppLang.instance.changeLanguage(lang);
