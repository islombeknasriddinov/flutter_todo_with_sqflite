...
Future<bool> startSyncWorker() async {
  ...
  final db = await openDatabase(
    await DarmonDatabase.getPath(),
    version: 2,
    onCreate: (db, version) => DarmonDatabase.onCreate.call(db),
    onDowngrade: (db, oldVersion, newVersion) => DarmonDatabase.onDowngrade.call(db),
  );

  DarmonDatabase instance = DarmonDatabase(db);
  await instance.syncDatabase(result);
