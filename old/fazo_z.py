...
class GenerateZ:
    ...
    def __update_row(self):
        ...
        b.append("\t// update row")
        b.append(
            "\tstatic Future<int> updateRow(Database db, " + t.get_name2class() + " row, {bool removeNull = false}) {")
    ...
    def __update_one(self):
        ...
        b.append(
            "\tstatic Future<int> updateOne(Database db, {" + ", ".join(
    ...
    def __save_row(self):
        ...
        b.append("\t// save row")
        b.append(
            "\tstatic Future<int> saveRow(Database db, " + t.get_name2class() + " row, {bool removeNull = false}) {")
        self.__gen_check_primary_name2class(prefix="row.")
        b.append("\t\tfinal data = row.toData();")
        b.append("\t\tif (removeNull) {")
        b.append("\t\t\tdata.removeWhere((key, value) => value == null);")
        b.append("\t\t}")
        b.append("\t\treturn db.insert(" + t.get_name2class() +
    ...
    def __save_one(self):
        ...
        method_params = [
            ...
            for c in t.columns]
        b.append("\t// save one")
        b.append("\tstatic Future<int> saveOne(Database db, {" + ", ".join(
            method_params) + ", bool removeNull = false}) {")
        self.__gen_check_primary_name2class()
        args = ", ".join([x.get_name2class() for x in t.columns])
        b.append(
    ...
    def __delete_all(self):
        ...
        b.append("\tstatic Future<int> deleteAll(Database db) {")
    ...
    def __delete_one(self):
        ...
        b.append("\tstatic Future<int> deleteOne(Database db, " + ", ".join(method_params) + ") {")
        self.__gen_check_primary_name2class()
        where_pk = ["${" + t.get_name2class() + "." + x.get_name2constant() + "} = ?" for x in
                    pk_columns]
        where_pk = " AND ".join(where_pk)
    ...
    def __insert_row_try(self):
        ...
        b.append("\tstatic Future<int> insertRowTry(Database db, " + t.get_name2class() + " row) {")
    ...
    def __insert_one_try(self):
        ...
        b.append(
            "\tstatic Future<int> insertOneTry(Database db, {" + ", ".join(method_params) + "}) {")
    ...
    def __insert_row(self):
        ...
        b.append("\tstatic Future<int> insertRow(Database db, " + t.get_name2class() + " row) {")
        self.__gen_check_required_name2class(prefix="row.")
        b.append(
    ...
    def __insert_one(self):
        ...
        b.append(
            "\tstatic Future<int> insertOne(Database db, {" + ", ".join(method_params) + "}) {")
        self.__gen_check_required_name2class()
        args = ", ".join([c.get_name2class() for c in t.columns])
